...
/**
 * @brief Construct a new Projection Factor:: Projection Factor object
 * 
 * @param _pts_i 路标点在起始帧（记为第i帧）中的归一化坐标
 * @param _pts_j 路标点在其他帧（记为第j帧）中的归一化坐标
 */
...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    TicToc tic_toc;
    // 下面是AddResidualBlock传入的5个量，但第5个量没有出现
    // 第一个量：第i帧的pq状态量，从imu到世界坐标系的
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);
    // 第二个量：第j帧的pq状态量，从imu到世界坐标系的
    ...
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;// 路标在第i帧相机下的3D坐标
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;// 路标在第i帧imu下的3D坐标
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;// 路标在世界坐标系下的3D坐标
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);// 路标在第j帧imu下的3D坐标
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic);// 路标在第j帧相机下的3D坐标
    ...
    residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>();// 重投影误差（2维），与手写vio中一模一样
#endif
    ...
    if (jacobians)
    {
        ...
        Eigen::Matrix<double, 2, 3> reduce(2, 3);// 重投影误差对路标在第j帧相机下的3D坐标的导数
#ifdef UNIT_SPHERE_ERROR
        ...
        // 下面是AddResidualBlock传入的前4个量的雅可比，具体赋值与手写vio一模一样
        if (jacobians[0])// 重投影误差（2维）对 i 时刻的状态量（7维）的雅可比（2*7）
        {
        ...
        // 重投影误差（2维）对 j 时刻的状态量（7维）的雅可比（2*7）
        if (jacobians[1])
        {
        ...
        // 重投影误差（2维）对 路标点的逆深度（1维）的雅可比（2*1）
        if (jacobians[3])
        {

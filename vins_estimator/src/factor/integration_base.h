...
class IntegrationBase
{
  public:
    ...
    /**
     * @brief 通过第0帧的pvq和两帧的测量值，预积分计算第1帧的pvq，并更新雅可比及协方差矩阵。需要注意的是pvq计算的开始时间是IntegrationBase对象创建的时间。
     * 比如pre_integrations[frame_count]对应的应该是最新的frame_count图像帧到来时开始积分计算pvq，那么到最后获得的pvq应该是从上一滑窗帧到当前帧的。
     * 而tmp_pre_integration应该是新的图像帧传入时构建的对象，那么就应该是从上一图像帧到当前图像帧的pvq
     * 
     * @param _dt 第0帧与第1帧之间的时间差，0在1之前
     * @param _acc_0 第0帧的加速度测量值
     * @param _gyr_0 第0帧的角速度测量值
     * @param _acc_1 第1帧的加速度测量值
     * @param _gyr_1 第1帧的角速度测量值
     * @param delta_p 第0帧的pvq
     * @param delta_q 
     * @param delta_v 
     * @param linearized_ba 第0帧的偏移
     * @param linearized_bg 
     * @param result_delta_p 第1帧的pvq
     * @param result_delta_q 
     * @param result_delta_v 
     * @param result_linearized_ba  第1帧的偏移
     * @param result_linearized_bg 
     * @param update_jacobian 是否更新雅可比
     */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // 通过离散运动方程计算第1帧的pvq
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);//第0帧加速度
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;// 第1帧角速度
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);// 第1帧旋转
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);// 第1帧加速度
        // 第1帧的pv
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt;
        result_delta_v = delta_v + un_acc * _dt;
        result_linearized_ba = linearized_ba;
        result_linearized_bg = linearized_bg;         
        //主要更新的时雅可比和协方差矩阵，公式与手写vio中的相同，其中的求导都是求的预积分量对各量的偏导数
        if(update_jacobian)
        {
    ...
    /**
     * @brief 通过上一帧的pvq和当前帧的测量值，计算当前帧的pvq
     * 
     * @param _dt 当前帧imu与上一帧imu的时间差
     * @param _acc_1 加速度测量值
     * @param _gyr_1 角速度测量值
     */
    ...
    /**
     * @brief 构建残差，是待优化量 - imu预积分量，与手写vio中的相同
     * 
     * @param Pi 
     * @param Qi 
     * @param Vi 
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        // 这里为何要再作修改得到corrected_delta_q呢
        // 这是对应的论文中因偏移变化需要重新积分的方法，corrected_delta_q就是bg变化后的修正量
        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);
        Eigen::Vector3d corrected_delta_v = delta_v + dv_dba * dba + dv_dbg * dbg;
        Eigen::Vector3d corrected_delta_p = delta_p + dp_dba * dba + dp_dbg * dbg;
        // imu的优化残差，是待优化量 - imu预积分量，与手写vio中的相同
    ...
    double dt; //当前帧与上一帧imu的时间差
    Eigen::Vector3d acc_0, gyr_0;// 上一帧的加速度和角速度测量值
    Eigen::Vector3d acc_1, gyr_1;// 当前帧的加速度和角速度测量值

    const Eigen::Vector3d linearized_acc, linearized_gyr;
    Eigen::Vector3d linearized_ba, linearized_bg;//不断传递的偏移
    ...
    double sum_dt; //从对象创建开始不断累计时间差的总和，与delta_pvq一样
    //通过积分获得从对象创建开始不断累计的pvq状态值，pre_integrations[frame_count]中就是两个图像帧之间的pvq（或者叫预积分量）
    // 而tmp_pre_integration中就是从第0帧到当前帧的pvq
    Eigen::Vector3d delta_p;
    Eigen::Quaterniond delta_q;
    Eigen::Vector3d delta_v;

    std::vector<double> dt_buf; //储存imu帧间的时间差
    std::vector<Eigen::Vector3d> acc_buf;// 储存所有imu帧的加速度
    std::vector<Eigen::Vector3d> gyr_buf;// 储存所有imu帧的角速度

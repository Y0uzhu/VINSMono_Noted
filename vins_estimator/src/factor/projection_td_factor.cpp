...
/**
 * @brief Construct a new Projection Td Factor:: Projection Td Factor object
 * 
 * @param _pts_i 特征点在初始帧的归一化坐标
 * @param _pts_j 特征点在当前帧的归一化坐标
 * @param _velocity_i 特征点的移动速度
 * @param _velocity_j 
 * @param _td_i 
 * @param _td_j 
 * @param _row_i 特征点的像素坐标v
 * @param _row_j 
 */
...
bool ProjectionTdFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    residual = (pts_camera_j / dep_j).head<2>() - pts_j_td.head<2>();// 构建的重投影残差
#endif

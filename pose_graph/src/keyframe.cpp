...
/**
 * @brief 构建关键帧，并额外检测特征提取表述子
 * 
 * @param _time_stamp 时间戳
 * @param _index 索引
 * @param _vio_T_w_i vio发来的关键帧位姿
 * @param _vio_R_w_i 
 * @param _image 关键帧图像
 * @param _point_3d 路标点在世界坐标系下的3D坐标
 * @param _point_2d_uv 路标点在某一帧下的像素坐标
 * @param _point_2d_norm 路标点在某一帧下的归一化坐标
 * @param _point_id 路标点的id
 * @param _sequence 序列号
 */
...
void KeyFrame::computeBRIEFPoint()
{
	...
	if(1)
		cv::FAST(image, keypoints, fast_th, true);// 提取fast特征
	else
	{
	...
	extractor(image, keypoints, brief_descriptors);// 获得描述子
	for (int i = 0; i < (int)keypoints.size(); i++)
	{
		Eigen::Vector3d tmp_p;
		m_camera->liftProjective(Eigen::Vector2d(keypoints[i].pt.x, keypoints[i].pt.y), tmp_p);// 像素坐标转3D坐标
		cv::KeyPoint tmp_norm;
		tmp_norm.pt = cv::Point2f(tmp_p.x()/tmp_p.z(), tmp_p.y()/tmp_p.z());// 归一化坐标
...
/**
 * @brief 关键帧中某个特征点的描述子与回环帧的所有描述子匹配，通过寻找最小汉明距离获得
 * 
 * @param window_descriptor 当前帧原有的一个特征描述子
 * @param descriptors_old 回环帧原有的特征描述子（所有）
 * @param keypoints_old 回环帧回环特征的二维坐标
 * @param keypoints_old_norm 回环帧回环特征的二维归一化坐标
 * @param best_match 与当前帧的一个特征描述子最佳匹配的回环帧特征的像素坐标（在回环帧下）
 * @param best_match_norm 最佳匹配的回环帧特征的归一化坐标（在回环帧下）
 * @return true 
 * @return false 
 */
...
/**
 * @brief   将关键帧与回环帧进行原有特征点的BRIEF描述子匹配
 * @param[out]  matched_2d_old  	回环帧匹配后的二维坐标
 * @param[out]  matched_2d_old_norm 回环帧匹配后的二维归一化坐标
 * @param[out]  status				匹配状态，成功为1
 * @param[in]   descriptors_old		回环帧新检测的描述子
 * @param[in] 	keypoints_old 		回环帧回环特征的二维坐标
 * @param[in] 	keypoints_old_norm	回环帧回环特征的二维归一化坐标
 * @return      void
*/
...
/**
 * @brief 通过RANSAC的PNP检验去除匹配异常的点，并通过pnp获得回环帧在世界坐标系下的位姿
 * 
 * @param matched_2d_old_norm 匹配点在回环帧的归一化坐标
 * @param matched_3d 匹配点的3D坐标
 * @param status 成功标志位
 * @param PnP_T_old 回环帧在世界坐标系下的平移
 * @param PnP_R_old 回环帧在世界坐标系下的旋转
 */
...
/**
 * @brief 寻找并建立当前帧与回环帧之间的匹配关系，通过汉明距离匹配特征点，
 * 之后通过pnp计算了回环帧的位姿（直接调的函数，不知道什么原理，而且这个回环帧的位姿只用于求解当前帧与回环帧的相对位姿，并没有修改回环帧自身的位姿）
 * 最后发布了匹配消息
 * 
 * @param old_kf 回环帧
 * @return true 
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	{
        //RANSAC PnP检测去除误匹配的点，并通过pnp获得回环帧在世界坐标系下的位姿
		status.clear();
...
/**
 * @brief 更新关键帧位姿和前端vio发来的位姿，更新vio_R_w_i和R_w_i
 * 
 * @param _T_w_i 
 * @param _R_w_i 
 */
...
/**
 * @brief 用输入的位姿信息更新当前帧与回环帧之间的相对位姿，但要求输入的位姿信息不是特别离谱
 * 
 * @param _loop_info 
 */

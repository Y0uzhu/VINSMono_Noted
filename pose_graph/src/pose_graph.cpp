...
/**
 * @brief 向keyframelist中添加关键帧，进行回环检测与闭环，计算漂移量的初值，发布了漂移修正后的位姿
 * 
 * @param cur_kf 要插入的关键帧，简称关键帧
 * @param flag_detect_loop 回环标志位（是否进行回环检测）
 */
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    //获取当前帧的位姿vio_P_cur、vio_R_cur并利用w_r_vio来更新，这个好像是用来修正不同图像序列间的漂移量的
    cur_kf->getVioPose(vio_P_cur, vio_R_cur);
    vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
    vio_R_cur = w_r_vio *  vio_R_cur;
    cur_kf->updateVioPose(vio_P_cur, vio_R_cur);// 用漂移修正完的位姿对关键帧的位姿进行更新，相当于把不同的图像序列转换到了一个下
    // 这里对帧索引又作了一次更新
    ...
    {
        TicToc tmp_t;
        loop_index = detectLoop(cur_kf, cur_kf->index);// 找到对应的回环帧
    }
    else
    {
        addKeyFrameIntoVoc(cur_kf);
    }
	if (loop_index != -1)// 找到了回环帧
	{
        //printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
        KeyFrame* old_kf = getKeyFrame(loop_index);//获取回环帧
        ...
        if (cur_kf->findConnection(old_kf))
        {
            ...
            // 返回当前帧与回环帧之间的相对位姿，这个位姿是在findConnection函数中通过pnp得到的
            ...
            //将所有图像序列都合并到世界坐标系下
            // 这段的意思感觉是如果当前帧与回环帧所处的序列不同，则通过漂移量来更新该序列所有帧的vio位姿
            // 但为啥更新过了则不再更新了呢？而且下面不是每帧都会更新vio位姿吗
            // 明白了，这是由于并非所有的帧都能构建回环，而一旦序列变化又没有构建新的回环，这部分帧就只能根据之前的漂移更新，会存在较大的误差
            // 所以就需要对之前的新序列的帧进行更新，这个循环相当于对上面用w_r_vio修正的补充
	...
	m_keyframelist.lock();
    // 利用vio的位姿和漂移来更新当前帧的位姿，这个漂移应该在优化中进行了修正
    Vector3d P;
    Matrix3d R;
...
/**
 * @brief 回环检测，计算与之前每一帧的相似度评分，返回评分大于0.015的最早的关键帧索引
 * 
 * @param keyframe 关键帧
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        int cur_index = -1;// 待优化帧索引
        int first_looped_index = -1;// 最早的全局回环帧
        m_optimize_buf.lock();
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                // 添加的节点居然是欧拉角和位移，这个欧拉角的参数是自定义的，只要了1维的
                ...
                //add edge
                // 把当前帧（第i帧）与其之前5帧中的每一帧（记为第j帧）都构建一个残差
                for (int j = 1; j < 5; j++)
                {
                ...
                if((*it)->has_loop)
                {
                    ...
                    Vector3d euler_conncected = Utility::R2ypr(q_array[connected_index].toRotationMatrix());// 第j帧的欧拉角
                    Vector3d relative_t;// t_j_i
                    relative_t = (*it)->getLoopRelativeT();
                    double relative_yaw = (*it)->getLoopRelativeYaw();// yaw_j_i
...
/**
 * @brief 更新关键帧的回环信息，返回通过两种方式获得的当前帧位姿的差值并更新r_drift和t_drift。
 * 这两种方式分别为：vio返回的当前帧位姿，用回环帧位姿*相对位姿获得的当前帧位姿
 * 
 * @param index 帧索引
 * @param _loop_info 8维量，位置*3，姿态*4，yaw角差*1
 */
void PoseGraph::updateKeyFrameLoop(int index, Eigen::Matrix<double, 8, 1 > &_loop_info)
{
    ...
    if (abs(_loop_info(7)) < 30.0 && Vector3d(_loop_info(0), _loop_info(1), _loop_info(2)).norm() < 20.0)
    {
        if (FAST_RELOCALIZATION)
        {
            ...
            Vector3d w_P_old, w_P_cur, vio_P_cur;
            Matrix3d w_R_old, w_R_cur, vio_R_cur;
            old_kf->getPose(w_P_old, w_R_old);// 获得回环帧位姿
            kf->getVioPose(vio_P_cur, vio_R_cur);// vio返回的当前帧位姿

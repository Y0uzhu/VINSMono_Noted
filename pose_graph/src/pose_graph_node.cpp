...
//世界坐标系下的路标点云，其包括points和channels两种一一对应的属性，points即世界系下的路标点，
// channels.values是一个5维向量，分别为路标点在某一帧下的归一化坐标[2]，像素坐标[2]和路标的id[1]。
queue<sensor_msgs::PointCloudConstPtr> point_buf;
queue<nav_msgs::Odometry::ConstPtr> pose_buf;// 关键帧的位姿
queue<Eigen::Vector3d> odometry_buf;// 相机的位置
std::mutex m_buf;
std::mutex m_process;// 相机外参
int frame_index  = 0;
int sequence = 1;
PoseGraph posegraph;//构建一个全局的位姿图优化对象
...
/**
 * @brief 关键帧的特征点云，每项包含一个points量（路标在世界坐标系下的坐标）和一个values量（5维向量，前两项为路标在起始帧的归一化坐标，34项为路标在起始帧的像素坐标，最后一项是路标的id）
 * 
 * @param point_msg 
 */
...
/**
 * @brief imu前向递推的回调函数，该函数通过接收前端IMU预积分的结果来计算回环中的cam位姿并加入cameraposevisual中，频率与imu频率相同。
 * 前端imu预积分是在优化结果的基础上进行的，但由于imu频率大于优化频率，所以总体误差偏大
 * 
 * @param forward_msg 
 */
void imu_forward_callback(const nav_msgs::Odometry::ConstPtr &forward_msg)
{
    if (VISUALIZE_IMU_FORWARD)
    {
        ...
        cameraposevisual.add_pose(vio_t_cam, vio_q_cam);// 向位姿图中添加位姿？
...
// 重定位消息经过前端重新的优化后，直接将发来的位姿作为对应帧与回环帧的相对位姿使用
// 在updateKeyFrameLoop中通过与vio的位姿对比来更新r_drift和t_drift
...
/**
 * @brief 根据前端优化后的vio位姿信息计算cam位姿并加入cameraposevisual中，并且发布了cam的轨迹
 * 
 * @param pose_msg 前端优化后的vio位姿信息，是w_T_i
 */
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                vector<cv::Point2f> point_2d_normal;// 路标点在某一帧下的归一化坐标
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 2000, imu_forward_callback);//IMU前向传播
    ros::Subscriber sub_vio = n.subscribe("/vins_estimator/odometry", 2000, vio_callback); //订阅里程计topic
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback);    //订阅图像img
    ros::Subscriber sub_pose = n.subscribe("/vins_estimator/keyframe_pose", 2000, pose_callback);    //订阅keyframe pose
    ros::Subscriber sub_extrinsic = n.subscribe("/vins_estimator/extrinsic", 2000, extrinsic_callback);    //订阅相机到IMU之间的外参
    ros::Subscriber sub_point = n.subscribe("/vins_estimator/keyframe_point", 2000, point_callback);//订阅点云topic
    ros::Subscriber sub_relo_relative_pose = n.subscribe("/vins_estimator/relo_relative_pose", 2000, relo_relative_pose_callback);//订阅重定位相对位姿
    ...
    keyboard_command_process = std::thread(command);//创建command线程，监听命令行中键盘的输入
